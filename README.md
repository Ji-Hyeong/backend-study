...
| `apps:auth` | 8084 | JWT, OIDC/Keycloak, RBAC/ReBAC/ABAC, resource owner |
...
`transaction`과 `concurrency`는 가볍게 실행할 수 있도록 기본 DB로 H2 인메모리 모드를 사용합니다. `cache`의 런타임 Cache-Aside 예제, `concurrency`의 Redis 분산 락 예제, `auth`의 refresh token 저장소는 Redis가 필요합니다. `auth`의 OIDC 예제는 Keycloak 컨테이너와 `keycloak` 프로필을 사용합니다. 실행 후 `/h2-console`에서 각각 `jdbc:h2:mem:transaction_study`, `jdbc:h2:mem:concurrency_study`로 접속할 수 있습니다.
